...
CREATE TABLE test_int AS (
	WITH 
		test_t AS (SELECT patid, eventdate, consid, staffid, enttype, medcode, data1, data2, data3, data4, data5, data6 FROM SOURCE.test),
		meds AS (SELECT read_code,"desc" description, medcode FROM SOURCE.medical),
		entities AS (SELECT enttype, description, data_fields FROM SOURCE.entity WHERE data_fields in (7,8,4)),
