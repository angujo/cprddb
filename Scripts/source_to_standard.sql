...
CREATE TABLE source_to_standard AS (
	WITH CONC AS (
		SELECT * FROM concept WHERE invalid_reason IS NULL 
	),
	CTE_VOCAB_MAP AS (
       ...
       FROM CONC C
		JOIN CONCEPT_RELATIONSHIP CR ON C.CONCEPT_ID = CR.CONCEPT_ID_1 AND CR.invalid_reason IS NULL AND lower(cr.relationship_id) = 'maps to'
